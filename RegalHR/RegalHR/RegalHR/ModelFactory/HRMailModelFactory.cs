...
namespace RegalHR.ModelFactory
{
    public class HRMailModelFactory
    {
        ...
        public void AbnSendMail()
        {
            ...
            foreach (var tmp in tmpList)
            {
                ...
                if (dt.Rows.Count == 1)
                {
                    ...
                    try
                    {
                        ...
                        //Mail_Send(new string[] { "squalllee@regalscan.com.tw" }, new string[] { }, "個人出勤異常通知!", Html);
            ...
            {
                string title = "";
                if (Company == "TPE") title = "台北-";
                if (Company == "TXG") title = "台中-";
                if (Company == "TNN") title = "台南-";
                if (Company == "SU") title = "蘇州-";

                Mail_Send(new string[] { "angel@regalscan.com.tw" }, new string[] { }, title + "員工出勤異常清單!", Html);
                //Mail_Send(new string[] { "squalllee@regalscan.com.tw" }, new string[] { }, title + "員工出勤異常清單!", Html);
        ...
        /// </summary>
        /// <returns></returns>
        public List<AttendanceModel> GetAttAbnList(string Company)
        {
            ...
            try
            {
                ...
                DataTable dt = DbAccess.ExecuteDataTable("EXEC SearchAttendanceTemp @ManageFlag,@Company,@DepNo,@EmpNo,@KeyWord,@SDATE,@EDATE,@EmpStatus,@LimitVal,@SkipVal",
                    ...
                    DataAccess.CreateParameter("Company", DbType.String, Company),
